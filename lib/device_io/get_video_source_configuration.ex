defmodule Onvif.DeviceIO.GetVideoSourceConfiguration do
  ...
  def request(uri, args, auth \\ :xml_auth),
    do: Onvif.DeviceIO.request(uri, args, auth, __MODULE__)

  def request_body(token) do
    element(:"s:Body", [
      element(:"tmd:GetVideoSourceConfiguration", [element(:"tmd:VideoSourceToken", token)])
    ])
  end
